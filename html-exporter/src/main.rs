...
fn render_chapter_citation_sidenote(
    ...
    if let Some(pdf) = pdf_href {
        write!(
            out,
            "<a class=\"chapter-citation-link chapter-citation-pdf\" href=\"{}\">Download as PDF</a>",
            escape_attr(pdf)
        )
        .unwrap();
    }
    write!(
        out,
        "<a class=\"chapter-citation-link chapter-citation-github\" href=\"{}\" aria-label=\"View chapter source on GitHub\">{}View on GitHub</a>",
        escape_attr(&chapter_source_github_url(chapter)),
        github_icon_svg()
    )
    .unwrap();
    write!(
...
fn chapter_source_github_url(chapter: &ChapterNav) -> String {
    format!("{GITHUB_REPOSITORY_URL}/blob/main/{}", chapter.source)
}
